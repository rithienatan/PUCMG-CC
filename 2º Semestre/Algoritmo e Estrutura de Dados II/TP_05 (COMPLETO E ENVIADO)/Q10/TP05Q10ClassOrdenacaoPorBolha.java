...
* Exercício: TP05Q10ClassOrdenacaoPorBolha
*/
...
class Lista
{
	...
	/**
	 * Ordenacao por numero de temporadas
	 */
	public void bolha()
	{
		for(int i = (n-1); i > 0; i--)
		{
			for(int j = 0; j < i; j++)
			{
				if(lista[j].getn_temporadas() > lista[j+1].getn_temporadas())
				{
					swap(j+1,j);
				}//end if
...
class TP05Q10ClassOrdenacaoPorBolha
{
	/**
	 * Método main
	 */
	...
	//--------------------------------------------------------------------------------
		...
		Arq.openWrite("541488_bolha.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + lista.getcomparacoes());
		Arq.close();
	}//end main
}//end TP05Q10ClassOrdenacaoPorBolha
