...
* Exercício: TP05Q09ClassOrdenacaoPorCountingsort
*/
...
class Lista
{
	...
	private void ordenacaoNome(int tamanho)
	{
		for(int i = tamanho; i < (n-1); i++)
		{
			...
			for(int j = (i+1); j < n; j++)
			{
				if(lista[menor].getn_temporadas() == lista[j].getn_temporadas() && lista[menor].getnome().compareTo(lista[j].getnome()) >= 1)
				{
					menor = j;
				}//end if
			}//end for
	...
	{
		int resp;
		Series maior = lista[0];
		
		for(int i = 0; i < n; i++)
		{
			if(maior.getn_temporadas() < lista[i].getn_temporadas())
			{
				maior = lista[i];
			}
			quantComparar = quantComparar + 1;
		}//end for
		resp = maior.getn_temporadas();

		return(resp);
	}//end getMaior()

	/**
	 * Ordenacao por numeor de temporadas
	 */
	public void countingsort()
	{
		...
		for(int i = 0; i < n; count[lista[i].getn_temporadas()]++, i++);

		for(int i = 1; i < count.length; count[i] += count[i-1], i++);

		for(int i = n-1; i >= 0; ordenado[count[lista[i].getn_temporadas()]-1] = lista[i], count[lista[i].getn_temporadas()]--, i--);
...
class TP05Q09ClassOrdenacaoPorCountingsort
{
	/**
	 * Método main
	 */
	...
	//--------------------------------------------------------------------------------
		...
		Arq.openWrite("541488_countingsort.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + lista.getcomparacoes());
		Arq.close();
	}//end main
}//end TP05Q09ClassOrdenacaoPorCountingsort
