 ...
 # Matrícula: 541488
 # Data: 08/04/2019
 # Trabalho de CG parte 1 - Algoritmo de Boids
...
try:
    ...
    print('Este programa requer a biblioteca do PyOpenGL')
    raise SystemExit
...
class leadBoid(object):
    "Cria o objeto Leader boid"

    def __init__(self):
        "Inicializa o boid"
        # todo: take keyword args like Peter
        self.color = Vector3(255.0, 0.0, 0.0)  # R G B
    ...
    #def updateLocation(self, novoX, novoY, novoZ):

    def update(self, leadX, leadY, leadZ):
        ...
        self.location = Vector3(leadX, leadY, leadZ) #atualiza a posição do objeto de acordo com as direct keys
        self.location = self.location + self.velocity * 0 # note += is buggy
...
class boid(object):
    ...
    def __init__(self):
        "Inicializa o boid"
        # todo: take keyword args like Peter
        self.color = random_Vector3(1.0)  # R G B
        self.location = Vector3(0.0, 0.0, 0.0)  # x y z
        self.velocity = random_Vector3(-1.0, 1.0)  # vx vy vz
        self.adjustment = Vector3(0.0, 0.0, 0.0)  # to accumulate corrections
    ...
    # update de local levando em consideração o lead boid
    def anotherTypeOfUpdateForBoid(self, boidX, boidY, boidZ):
        "move to new position"
        velocity = self.velocity + self.adjustment  # note: += is buggy
        # Hack: Add a constant velocity in whatever direction
        # they are moving so they don't ever stop.
        if velocity.length() > 0:
            velocity = velocity + (velocity.normalize()
                                   * random_range(0.0, 0.007))
        self.velocity = velocity
        self.limit_speed(1.0)
        boidX = random.uniform(boidX-20.0, boidX+20.0)
        boidY = random.uniform(boidY-20.0, boidY+20.0)
        boidZ = random.uniform(boidZ-20.0, boidZ+20.0)
        self.location = Vector3(boidX, boidY, boidZ)
        self.location = self.location + self.velocity  # note += is buggy
...
class flock(object):
    ...
    def __init__(self, num_boids, num_boidsLead, cube0, cube1):
        "initialize a flock of boids"
        self.cube0 = cube0  # cube min vertex
        self.cube1 = cube1  # cube max vertex
        self.leadOfBoids = [] #boid leader
        for _ in itertools.repeat(None, num_boidsLead):
            self.leadOfBoids.append(leadBoid())
        self.boids = []
    ...
    def render(self):
        ...
        self.render_cube() # cubo

        # linha para representar lleader boids

        for leadBoid in self.leadOfBoids:
            gl.glPointSize(5)
            gl.glBegin(gl.GL_POINTS)
            gl.glColor(leadBoid.color.x, leadBoid.color.y, leadBoid.color.z)
            gl.glVertex(leadBoid.location.x, leadBoid.location.y, leadBoid.location.z)
            if leadBoid.velocity.length() > 0:
                ## head = boid.location + boid.velocity.normalize()
                head = leadBoid.location + leadBoid.velocity.normalize() * 3
            else:
                head = leadBoid.location
            gl.glVertex(head.x, head.y, head.z)
            gl.glEnd()
            gl.glPointSize(1)

        # linha para representar os outros boids
    ...
    def update(self, leadX, leadY, leadZ):
        ...
        for leadBoid in self.leadOfBoids:
            leadBoid.orient(self.leadOfBoids)  # Calcula a nova velocidade
        for leadBoid in self.leadOfBoids:
            leadBoid.update(leadX, leadY, leadZ)  # Mover para a nova posição
            leadBoid.wrap(self.cube0, self.cube1)  # Implementa hypertaurus

        for boid in self.boids:
            boid.orient(self.leadOfBoids)  # Calcula a nova velocidade
        for boid in self.boids:
            #leadY = random.uniform(-99, 99)
            #boid.anotherTypeOfUpdateForBoid(leadX, leadY, leadZ)
...
def main():
    ...
    angle = 0.0  # ângulo de rotação da câmera
    ## side = 700
    ...
    theflock = flock(num_boids, num_boidsLead, cube0, cube1)

    #posição do lead boid
    leadX = 0.0
    leadY = 0.0
    leadZ = 0.0

    # posição da camera | valores 0, 1 e 2
    change = 0
    ...
    while True:
        ...
        if (event.type == pygame.KEYDOWN):
                ...
                if (event.key == pygame.K_6):
                    ...
                    separation_entry.delete(0, END)
                    separation_entry.insert(0, str(round(f_separation, 6)))
                # atualiza a posição do boid principal
                if (event.key == pygame.K_UP):
                    leadY = leadY + 5.0
                    theflock.update(leadX, leadY, leadZ)
                ...
                if (event.key == pygame.K_RIGHT):
                    leadX = leadX - 5.0
                    theflock.update(leadX, leadY, leadZ)
                # atualiza a quantidade de boids
                if (event.key == pygame.K_m):
                    num_boids = num_boids + 10
                    theflock = flock(num_boids, num_boidsLead, cube0, cube1)
                if (event.key == pygame.K_n):
                    if(num_boids >= 10):
                        num_boids = num_boids - 10
                    elif(num_boids > 0 and num_boids <10):
                        num_boids = 0
                    theflock = flock(num_boids, num_boidsLead, cube0, cube1)
                # posições de camera
