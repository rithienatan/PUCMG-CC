...
* Exercício: TP05Q07ClassOrdenacaoPorHeapsort
*/
...
class Lista
{
	...
	private void ordenacaoNome(int tamanho)
	{
		for(int i = tamanho; i < (n-1); i++)
		{
			...
			for(int j = (i+1); j < n; j++)
			{
				if(lista[menor].getformato().trim().compareTo(lista[j].getformato().trim()) == 0 && lista[menor].getnome().compareTo(lista[j].getnome()) >= 1)
				{
					menor = j;
				}//end if
			}//end for
	...
	/**
	 * Metodo de ordenacão por formato
	 */
	public void heapsort()
	{
		...
		//voltando a lista com a posicao 0
		tmp = lista;
		lista = new Series[n];
		for(int i = 0; i < n; i++)
		{
	...
	/**
	 * Constroi heap
	 * @param tam Recebe o tamanho do heap
	 */
	private void constroi(int tam)
	{
		for(int i = tam; i > 1 && lista[i].getformato().trim().compareTo(lista[i/2].getformato().trim()) > 0; i /= 2)
		{
			swap(i/2, i);
	...
	/**
	 * Ordenar o heap construido
	 * @param tam Recebe o tamanho mais ou menos ordenado do heap
	 */
	private void reconstroi(int tam)
	{
		...
		while(i <= (tam/2))
		{
			if(lista[2*i].getformato().trim().compareTo(lista[2*i+1].getformato().trim()) > 0 || 2*i == tam)
			{
				filho = 2*i;
			}
			else
			...
			if(lista[i].getformato().trim().compareTo(lista[filho].getformato().trim()) < 0)
			{
				swap(i, filho);
				i = filho;
			}
...
class TP05Q07ClassOrdenacaoPorHeapsort
{
	/**
	 * Método main
	 */
	...
	//--------------------------------------------------------------------------------
		...
		Arq.openWrite("541488_heapsort.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + lista.getcomparacoes());
		Arq.close();
	}//end main
}//end TP05Q06ClassOrdenacaoPorHeapsort
