...
* Exercício: TP05Q12ClassOrdenacaoPorRadixsort
*/
...
class Lista
{
	...
	/**
	 * Pega o maior numero na lista
	 * @param list
	 * @param n
	 * @return mx
	 */
	private int getMax(Series []list, int x)
	{
		int mx = (list[0].getn_episodios()*1000) + list[0].getn_temporadas();
		for(int i = 1; i < n; i++)
		{
			if((list[i].getn_episodios()*1000) + list[i].getn_temporadas() > mx)
			{
				mx = (list[i].getn_episodios()*1000) + list[i].getn_temporadas();
			}//end if
	 ...
	 * @param exp
	 */
	private void countSort(Series list[], int n, int exp)
	{
		Series output[] = new Series[n];
		int i;
		...
		for(i = 0; i < n; i++)
		{
			count[(((list[i].getn_episodios()*1000) + list[i].getn_temporadas())/exp)%10]++;
		}//end for
		...
		for(i = n-1; i >= 0; i--)
		{
			output[count[(((list[i].getn_episodios()*1000) + list[i].getn_temporadas())/exp)%10]-1] = list[i];
			count[(((list[i].getn_episodios()*1000) + list[i].getn_temporadas())/exp)%10]--;
		}//end for
	 ...
	 * @param list Recebe o array lista
	 * @param x quantidade de elementos que quero ordenar no array
	 */
	public void radixSort(Series []list, int x)
	{
		int m = getMax(list, x);
...
class TP05Q12ClassOrdenacaoPorRadixsort
{
	/**
	 * Método main
	 */
...
}//end TP0512ClassOrdenacaoPorRadixsort
