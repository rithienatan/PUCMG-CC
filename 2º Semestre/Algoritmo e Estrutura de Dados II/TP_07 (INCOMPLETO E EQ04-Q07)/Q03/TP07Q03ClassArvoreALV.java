...
* Exercício: TP07Q03ClassArvoreAVL
*/
...
class No
{
	...
	public No(Series x, No esq, No dir, int nivel)
	{
		this.elemento = x;
		this.esq = esq;
		this.dir = dir;
...
class ArvoreAVL
{
	...
	private int nivel(No no)
	{
		...
		{
			resp = 1 + Math.max(nivel(no.esq), nivel(no.dir));
		}
		return(resp);
	}//end nivel
	...
	private No balancear(No i)throws Exception
	{
		if(i != null)
		{
			...
			{
				int fatorFilhoEsq = No.getNivel(i.esq.dir) - i.getNivel(i.esq.esq);
			...
			{
				throw new Exception("Erro fator de balanceamento");
			}//end if
		}//end if
