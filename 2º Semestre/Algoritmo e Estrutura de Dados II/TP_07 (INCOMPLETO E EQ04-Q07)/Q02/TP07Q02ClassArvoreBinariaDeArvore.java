...
* Exercício: TP07Q02ClassArvoreBinariaDeArvore
*/
...
class No2
{
	...
	public No2(String x, No2 esq, No2 dir)
	{
		this.nomeSerie = x;
		this.esq = esq;
		this.dir = dir;
...
class No
{
	...
	public No(char x, No esq, No dir, No2 raiz2)
	{
		this.letra = x;
		this.esq = esq;
		this.dir = dir;
...
class ArvoreDeArvore
{
	...
	/**
	 * inserir letras na Arvore
	 * @param x Elemento a ser inserido
	 */
	private void inserirletra(char x) throws Exception
	{
		raiz = inserirletra(x, raiz);
	}//end inserir()

	private No inserirletra(char x, No i) throws Exception
	{
		if(i == null)
		{
	...
	/**
	 * Mostrar na primeira árvore para passar em todos os elementos
	 */
	...
	private boolean pesquisar(String x, No i)
	{
		...
		else if(x.charAt(0) == i.letra)
		{
			quantComparar = quantComparar + 2;
			System.out.print("raiz ");
			resp = pesquisar2(x, i.raiz2);
		}
		else if(x.charAt(0) < i.letra)
		{
			quantComparar = quantComparar + 3;
	...
	private boolean pesquisar2(String x, No2 s)
	{
		...
		else if(x.replaceAll(" ", "").equals(s.nomeSerie.replaceAll(" ", "")) == true)
		{
			quantComparar = quantComparar + 2;
			resp = true;
		}
		else if(x.compareTo(s.nomeSerie) < 0)
		{
			quantComparar = quantComparar + 3;
			System.out.print("esq ");
			resp = pesquisar2(x, s.esq);
		}
		else
		{
			quantComparar = quantComparar + 3;
			System.out.print("dir ");
			resp = pesquisar2(x, s.dir);
		}//end if
	...
	private No inserir(Series x, No i) throws Exception
	{
		...
		else if(x.getnome().charAt(0) == i.letra)
		{
			quantComparar = quantComparar + 2;
			i.raiz2 = inserirArvore2(x, i.raiz2);
		}
		else if(x.getnome().charAt(0) < i.letra)
		{
			quantComparar = quantComparar + 3;
	...
	private No2 inserirArvore2(Series x, No2 i) throws Exception
	{
		if(i == null)
		{
			quantComparar = quantComparar + 1;
		...
		else if(x.getnome().compareTo(i.nomeSerie) < 0)
		{
			quantComparar = quantComparar + 2;
	...
	private No remover(String x, No i) throws Exception
	{
		...
		{
			quantComparar = quantComparar + 2;
			i.raiz2 = removerArvore2(x, i.raiz2);
		}
		else if(x.charAt(0) < i.letra)
		{
			quantComparar = quantComparar + 3;
	...
	/**
	 * Remover na segunda arvore própriamente dito
	 */
	private No2 removerArvore2(String x, No2 i) throws Exception
	{
		if(i == null)
		{
			quantComparar = quantComparar + 1;
		...
		else if(x.compareTo(i.nomeSerie) < 0)
		{
			quantComparar = quantComparar + 2;
			i.esq = removerArvore2(x, i.esq);
		}
		else if(x.compareTo(i.nomeSerie) > 0)
		{
			quantComparar = quantComparar + 3;
			i.dir = removerArvore2(x, i.dir);
		}
		else if(i.dir == null)
		{
			quantComparar = quantComparar + 4;
		...
		{
			quantComparar = quantComparar + 5;
			i.esq = anterior2(i, i.esq);
		}//end if

		return(i);
	}//end inserir()

	private No2 anterior2(No2 i, No2 j)
	{
		if(j.dir != null)
		{
			j.dir = anterior2(i, j.dir);
		}
		else
		{
			i.nomeSerie = j.nomeSerie; j = j.esq;
		}//end if
...
class TP07Q02ClassArvoreBinariaDeArvore
{
	/**
	 * Método main
	 */
	...
	//---------------------------------------------------------------------------------
		...
		Series series = null;
		ArvoreDeArvore aa = new ArvoreDeArvore();
		...
		do
		{
			...
			{
				if(aa.pesquisar(arq[quantEntrada]) == true)
				{
	...
	//--------------------------------------------------------------------------------
		...
		Arq.openWrite("541488_arvoreArvore.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + aa.getcomparacoes());
		Arq.close();
	}//end main
}//end TP07Q02ArvoreBinariaDeArvore
