...
* Exercício: TP05Q08ClassOrdenacaoPorQuicksort
*/
...
class Lista
{
	...
	private void ordenacaoNome(int tamanho)
	{
		for(int i = tamanho; i < (n-1); i++)
		{
			...
			for(int j = (i+1); j < n; j++)
			{
				if(lista[menor].getpais_origem().trim().compareTo(lista[j].getpais_origem().trim()) == 0 && lista[menor].getnome().compareTo(lista[j].getnome()) >= 1)
				{
					menor = j;
				}//end if
			}//end for
	...
	{
		int i = esq, j = dir;
		Series pivo = lista[(dir + esq)/2];
		while(i <= j)
		{
			while(lista[i].getpais_origem().trim().compareTo(pivo.getpais_origem().trim()) <= -1)
			{
				quantComparar = quantComparar + 1;
				i++;
			}//end while
			while(lista[j].getpais_origem().trim().compareTo(pivo.getpais_origem().trim()) >= 1)
...
class TP05Q08ClassOrdenacaoPorQuicksort
{
	/**
	 * Método main
	 */
	...
	//---------------------------------------------------------------------------------
		...
		lista.quicksort();
		//lista.ordenacaoNome(0);
		lista.mostrar();
	//--------------------------------------------------------------------------------
		long fim = System.currentTimeMillis();

		Arq.openWrite("541488_quicksort.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + lista.getcomparacoes());
		Arq.close();
	}//end main
}//end TP05Q08ClassOrdenacaoPorQuicksort
