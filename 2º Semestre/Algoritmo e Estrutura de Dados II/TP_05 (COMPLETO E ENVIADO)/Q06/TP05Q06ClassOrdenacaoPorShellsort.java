...
* Exercício: TP05Q06ClassOrdenacaoPorShellsort
*/
...
class Lista
{
	...
	/**
	 * Metodo de ordenacão por idioma
	 */
	...
	/**
	 * Metodo de ordenação por idioma
	 */
	private void insercaoPorCor(int cor, int h)
	{
		for(int i = (h + cor); i < n; i+= h)
		{
			...
			while((j >= 0) && (lista[j].getidioma_original().trim().compareTo(tmp.getidioma_original().trim()) > 0))
			{
				lista[j+h] = lista[j];
				j-=h;
			}//end while
...
class TP05Q06ClassOrdenacaoPorShellsort
{
	/**
	 * Método main
	 */
	...
	//--------------------------------------------------------------------------------
		...
		Arq.openWrite("541488_insercao.txt");
		Arq.println("541488\t" + (fim-inicio) + "ms\t" + lista.getcomparacoes());
		Arq.close();
	}//end main
}//end TP05Q06ClassOrdenacaoPorShellsort
